...
namespace SpotifyDashboard.Server.Endpoints
{
    public static class UserEndPoint
    {
        ...
        private static async Task<IEnumerable<Playlist>> GetUserPlaylistsAsync([FromHeader(Name = "Authorization")] string token, DashboardService data)
        {
            var playlists = await data.GetUserPlaylist();
            return playlists;
        }
        ...
        private static async Task<User> GetUserDataAsync([FromHeader(Name = "Authorization")] string token , DashboardService data)
        {   
